...
class Goods extends Home{
    ...
    public function lists(){		    			   	 	 
		...
		$this->assign('brandlist', $brandlist);
		 ...
		 //属性列表
		$attr_list=model("Attributes")->getList($info["model_id"]);
     ...
     public function detail($id){
		...
		$this->assign("ids",$ids);
		 ...
		 if($info["specs"]){
			...
			$map["model_id"]=$cate["model_id"];
			$map["pid"]=0;
			$attrlist=db("specs")->where($map)->select();
			$array=explode(",",$info["specs"]);
			foreach ($attrlist as $n=>$v){
				      $where["pid"]=$v["id"];
                      $where["title"]=array("in",$array);
    ...
    public function comment(){		    			   
		...
		//查询条件is_picture
		$is_picture=(int)input("is_picture");	
