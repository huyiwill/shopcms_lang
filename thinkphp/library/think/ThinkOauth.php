...
abstract class ThinkOauth{
    ...
    /*    $config = C("THINK_SDK_{$this->Type}");*/
		...
		$APP_KEY=str_replace("think\sdk\\","",$type.'KEY');
		//dump($APP_KEY);
		$APP_SECRET=str_replace("think\sdk\\","",$type.'Secret');
    ...
    public static function getInstance($type, $token = null) {
        ...
        require_once(dirname(__FILE__). "\sdk". "\\".$name.".php");
		$name="\\think\sdk\\".$name;
    ...
    private function config(){
       ...
       $type=str_replace("THINK\SDK\\","",$this->Type) ;
       
      $this->Callback = "http://$_SERVER[HTTP_HOST]".url('Sync/callback',array('type'=>strtolower($type)));
